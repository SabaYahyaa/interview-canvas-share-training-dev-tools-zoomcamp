...
const MOCK_SESSION: InterviewSession = {
  ...
  state: "active",
  candidate_editing_enabled: true, // Ensure editing is explicitly enabled
...
const MOCK_PARTICIPANT: Participant = {
  ...
  role: "owner", // Ensure role is 'owner' or 'interviewer'
...
async function request<T>(
  ...
  console.log(`[MOCK API] ${options.method ?? "GET"} ${safePath}`, options.body ?? "");
  ...
  if (cleanPath === "/v1/sessions") {
    if (options.method === "POST") {
      ...
      return {
        ...MOCK_SESSION,
        title: body.title || MOCK_SESSION.title,
        prompt: body.prompt || MOCK_SESSION.prompt,
        duration_minutes: body.duration_minutes || 60,
      } as T;
    }
    return [{ ...MOCK_SESSION, participants: [MOCK_PARTICIPANT], link: null }] as T;
  }

  // Session detail (/v1/sessions/:id)
  if (cleanPath.match(/^\/v1\/sessions\/[^/]+$/)) {
    if (options.method === "PATCH") {
      const patch = JSON.parse((options.body as string) ?? "{}");
      return { ...MOCK_SESSION, ...patch } as T;
    }
    return {
  ...
  if (cleanPath.match(/^\/v1\/sessions\/[^/]+\/(start|end|archive|duplicate)$/)) {
    return MOCK_SESSION as T;
  }

  // Canvas document routes
  if (cleanPath.match(/^\/v1\/sessions\/[^/]+\/canvas$/)) {
    if (options.method === "PUT") {
      return "saved" as T;
    }
    return MOCK_CANVAS as T;
  }

  // Participants
  if (cleanPath.match(/^\/v1\/sessions\/[^/]+\/participants$/)) {
    return MOCK_PARTICIPANT as T;
  }

  // Guest links
  if (cleanPath.match(/^\/v1\/sessions\/[^/]+\/guest-links$/)) {
    return {
      id: "link-123",
      session_id: MOCK_SESSION.id,
  ...
  // Audit event logs
  if (cleanPath.match(/^\/v1\/sessions\/[^/]+\/audit$/)) {
    return [] as T;
  }
...
// --- MOCK REALTIME WEBSOCKET MANAGEMENT ---
...
async function connect(sessionId: string, room: RoomConnection) {
  ...
  // Bypasses actual WebSocket connection when testing in mock mode
...
export const api = {
  ...
  inspectToken: (token: string) =>
    request<TokenInspection>(`/v1/join/${encodeURIComponent(token)}`, { method: "GET" }, false),
